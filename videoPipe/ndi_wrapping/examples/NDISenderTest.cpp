...
struct MyApp : public App {
  ...
  void onCreate() {
    ...
    if (ndiSender.init("NDISenderTest", NDISender::VideoConfig())) {
  ...
  void onDraw(Graphics& g) {
    ...
    if (ndiSender.sendDirect(fbo.tex())) {
      if (frameCount % 60 == 0) {  // Print every 60 frames (roughly 1 second at 60fps)
        cout << "Sent NDI frame " << frameCount << endl;
      }
    } else {
